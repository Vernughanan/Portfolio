Professional Portfolio — HTML/CSS/JS

Live Demo 

vernuportfolio.netlify.app

How to view

1. Open `index.html` in your browser (double-click or use Live Server extension).

Notes

- This is a single-page, responsive portfolio using only HTML, CSS and JavaScript.
- Contact form is demo-only and doesn't send messages; replace with your backend or use Formspree/EmailJS.
 - A placeholder CV PDF is included at `assets/J.VERNUGHANAN Resume.pdf`. Replace it with your real CV; the "Download CV" button in the navigation will download this file.
